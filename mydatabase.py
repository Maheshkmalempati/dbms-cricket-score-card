...
class Database:
    ...
    # Add your database operations methods here

# Rest of your code for GUI and application logic
...
class Database:
    ...
    def createTable(self):
        ...
        self.foreign.execute("PRAGMA foreign_keys = ON")
        self.foreign.execute('''CREATE TABLE IF NOT EXISTS partnership (
                                BATSMAN1_ID INTEGER,
                                BATSMAN2_ID INTEGER,
                                ...
                                FOREIGN KEY (BATSMAN1_ID) REFERENCES PLAYER(player_id) ON DELETE SET NULL,
                                FOREIGN KEY (BATSMAN2_ID) REFERENCES PLAYER(player_id) ON DELETE SET NULL,
                                FOREIGN KEY (INNINGS_ID) REFERENCES RESULT(m_id) ON DELETE SET NULL
                                );''')
    ...
    def insertPlayer(self,values):
        try:
            self.playerCursor.execute('''INSERT INTO PLAYER(player_id,TeamID,playername, noofmatches,noofruns,noofwickets,jerseyno) VALUES(?,?,?,?,?,?,?);''', values)
                ...
                mb.showerror('Warning', "Error inserting into the PLAYER table")
        else:
            self.conn.commit()
    ...
    # Create tables (you might want to call this method if the tables don't exist)
                                    db.createTable()

    # Example: Insert a player
                                    player_data = (1, 1, 'Player1', 10, 500, 20, 23)
                                    db.insertPlayer(player_data)
    ...
    def insertPartnership(self, values):
        try:
            self.foreign.execute('''INSERT INTO partnership(BATSMAN1_ID, BATSMAN2_ID, PARTNERSHIP_ID, RUNS_SCORED, INNINGS_ID, OVERS)
                                   VALUES(?, ?, ?, ?, ?, ?);''', values)
        except:
            mb.showerror('Warning', "Error inserting into the partnership table")
        self.foreign.execute('''commit;''')
